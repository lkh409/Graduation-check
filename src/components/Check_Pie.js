...
export default class PieCheck extends PureComponent {
  render() {
    ...
    return (
      <ResponsiveContainer width={200} height={160}> {/*recharts에서 기본적으로 제공하는 컴포넌트, 차트 크기 반응형으로 조정 */}
        <PieChart> {/*원형 차트 컨테이너 */}
          <Pie // 원형 차트를 그리는 컴포넌트
          ...
          >
            {data.map((entry, index) => ( // data 배열의 각 요소 순회하고, 각 요소에 대해 새로운 Cell 컴포넌트 생성 후 반환. Cell 컴포넌트는 차트의 각 부분을 나타내고 색상은 COLORS배열에서 결정
